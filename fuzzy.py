...
def defuzzified_result(speed, distance):
    ...
    ax0.plot(x_power, power_vlo, 'm', linewidth=0.5, linestyle='--')
    ax0.plot(x_power, power_lo,  'r', linewidth=0.5, linestyle='--')
    ax0.plot(x_power, power_op,  'y', linewidth=0.5, linestyle='--')
    ax0.plot(x_power, power_hi,  'c', linewidth=0.5, linestyle='--')
    ax0.plot(x_power, power_vhi, 'g', linewidth=0.5, linestyle='--')
    ax0.fill_between(x_power, power0, aggregated, facecolor='Cyan', alpha=0.7)
    ax0.plot([power, power], [0, power_activation], 'k', linewidth=1.5, alpha=0.9)
    ax0.set_title('Ocena (wynik)')
    ...
    # Encode to base64 and savefig in stringIO
    img = StringIO.StringIO()  
    plt.savefig(img, format='png', transparent=True)
    plt.close()
    img.seek(0) 
    plot_url = base64.b64encode(img.getvalue()) 
    return plot_url, power
